...
format:
    beamer:
        ...
        header-includes:
          - \URL{https://www.hclwizard.org/}
          - \headerimage{uibk-stripes.png}
...
\only<2>{\includegraphics[width=0.95\textwidth]{slides_files/figure-beamer/blues2-spectrum-1.pdf}}
\only<3>{\includegraphics[width=0.95\textwidth]{slides_files/figure-beamer/blues3-spectrum-1.pdf}}
\only<4>{\includegraphics[width=0.95\textwidth]{slides_files/figure-beamer/blues-spectrum-1.pdf}}
:::
...
cols <- rev(c("#000000", "#00a9d1", "#29416e", "#e58c35", "#dd4048", "#942324"))
tinyplot(Mean ~ Year | Scenario, data = d, type = "l", lwd = 3, theme = "clean2",
  main = title, xlab = NA, ylab = bquote("Temperature change ["~degree~C~"]"),
  col = cols)

tinyplot(Mean ~ Year | Scenario, data = d, type = "l", lwd = 3, theme = "clean2",
  main = title, xlab = NA, ylab = bquote("Temperature change ["~degree~C~"]"),
  col = cols, legend = FALSE, xlim = c(1950, 2110))
tinyplot_add(data = subset(d, Year == max(Year)), type = "text",
  labels = paste(head(unique(d$Scenario), -1), "°C"), pos = 4, xpd = TRUE)
...
cols <- rev(c("#000000", diverging_hcl(5, "Blue-Red", rev = TRUE, l2 = 70)))
tinyplot(Mean ~ Year | Scenario, data = d, type = "l", lwd = 3, theme = "clean2",
  main = title, xlab = NA, ylab = bquote("Temperature change ["~degree~C~"]"),
  col = cols, legend = FALSE, xlim = c(1950, 2110))
tinyplot_add(data = subset(d, Year == max(Year)), type = "text",
...
cols <- rev(c("#000000", sequential_hcl(7, "ag_Sunset")[3:7]))
tinyplot(Mean ~ Year | Scenario, data = d, type = "l", lwd = 3, theme = "clean2",
  main = title, xlab = NA, ylab = bquote("Temperature change ["~degree~C~"]"),
  col = cols, legend = FALSE, xlim = c(1950, 2110))
tinyplot_add(data = subset(d, Year == max(Year)), type = "text",
