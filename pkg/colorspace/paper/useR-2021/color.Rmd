...
abstract: >
  Prior to R 4.0.0 R has had a poor default palette (using highly saturated red, green, blue, etc.)
  and only provided very few readily available alternative palettes, most of which also had poor
  perceptual properties (like the infamous rainbow palette). Starting with R 4.0.0 R has gained a
  ...
  the perceptual hue, chroma, luminance (HCL) properties in the palette. Here, an overview of the
  functions is given along with advice which palettes are particularly well-suited for which tasks,
  especially with regard to making them accessible to viewers with color vision deficiencies.
  This should facilitate adopting the functions in particular for setting better color defaults,
  both in daily work or in visualization functions/packages, etc.
preamble: |
...
overview of the palettes that are available and 
...
reference.
Perceptual balance and colorblind-safety are closely linked
...
`colorspace::swatchplot(pal, cvd = TRUE)`, where `pal` is a palette of colours.
More elaborate tools
are provided by the package \pkg{colorblindcheck}.
...
The table below summarises the main types of palettes and provides
suggestions for good default palettes for each type.

: An overview of the new palette functionality:  For each main type of palette, the Purpose Row describes what sort of data the type of palette is appropriate for, the Generate row gives the functions that can be used to generate palettes of that type, the List row names the functions that can be used to list available palettes, and the Robust row identifies one or two good default palettes of that type. 

+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
|          | Qualitative                           | Sequential                          | Diverging                                     |
+:=========+:======================================+:====================================+:==============================================+
| Purpose  | Categorical data                      | Ordered or numeric data             | Ordered or numeric data with a central value  |
|          |                                       | (high $\rightarrow$ low)            | (high $\leftarrow$ neutral $\rightarrow$ low) |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
| Generate | `palette.colors()`,                   | `hcl.colors()`                      | `hcl.colors()`                                |
|          | `hcl.colors()`                        |                                     |                                               |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
| List     | `palette.pals()`,                     | `hcl.pals("sequential")`            | `hcl.pals("diverging")`,                      |
|          | `hcl.pals("qualitative")`             |                                     | `hcl.pals("divergingx")`                      |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
| Robust   | `"Okabe-Ito"`                         | `"Viridis"`, `"YlnGnBu"`            | `"Purple-Green"`, `"Blue-Red 3"`              |
|          |                                       |                                     |                                               |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
