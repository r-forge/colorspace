...
abstract: >
  ...
  functions along with advice which palettes are well-suited for specific tasks,
...
```{r}
hcl.colors(7, palette="Purple-Green")
```
```{r PurpleGreen}
#| echo: FALSE
#| fig.width: 6
#| fig.height: .5
#| out.width: '100%'
paletteGrids(list(' '=hcl.colors(7, palette="Purple-Green")), qual=FALSE)
```
...
#| fig.height: .5
#| out.width: '100%'
paletteGrids(list(' '=hcl.colors(5, palette="Viridis")), qual=FALSE)
```
...
the new `"R4"` default and another new qualitative palette
...
#| fig.pos: 'ht!'
#| fig.show: 'hold'
#| fig.cap: 'Probability of wind speeds $>$ 39\,mph (63\,km\,h$^{-1}$) during hurricane Dorian in 2019.  Top-left is the original image and top-right is an emulation of how the original image might appear to a colorblind viewer.  Bottom-left is a reproduction using the `"OrRd"` palette, with a colorblind emulation at bottom-right.'
library("cowplot")
ggdraw() + 
...
ggdraw() + 
  draw_image("images/dorian-useR-2021-orrd10.png", width = 0.5) + 
  draw_image("images/dorian-useR-2021-orrd10-deutan.png", width = 0.5, x = 0.5)
```
...
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
|_Robust_  | `"Okabe-Ito"`                         | `"Blues 3"`, `"Viridis"`            | `"Purple-Green"`, `"Blue-Red 3"`              |
|          |                                       |                                     |                                               |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
