...
abstract: >
  ...
  **Meta:**
  ...
  Compile document in both HTML and PDF with `rmarkdown::render("color.Rmd")`.
  More details at <https://rjournal.github.io/rjtools/>.  
preamble: |
...
library("cowplot")
ggdraw() + 
  draw_image("images/color-demo-cat-1.png", width = 0.66) + 
  draw_image("images/color-demo-cont-1.png", width = 0.33, x = 0.66)
```
...
#| fig.align: 'center'
#| fig.pos: 'ht!'
#| fig.cap: 'Time series line plot of \code{EuStockMarkets}. Rows: Old \code{"R3"} default palette (top), new \code{"R4"} default palette (middle), \code{"OkabeIto"} palette (bottom), designed to be robust under color vision deficiencies. Columns: Normal vision (left) and emulated deuteranope vision (right). A color legend is used in the first row and direct labels in the other rows.'
...
#| fig.align: 'center'
#| fig.pos: 'ht!'
#| fig.cap: 'New base R palettes available from the \code{palette.colors()} function.  The label above each swatch shows the argument to provide to \code{palette.colors()} to produce the set of colors.  The palette at top-left is the new default that is also produced by \code{palette()}.  The "Okabe-Ito" palette is the default that is produced by \code{palette.colors()} (with no arguments).'
...
The `hcl.colors()` function was added in R 3.6.0.
...
selecting colors from is \emph{not} a fixed set of colours.  Instead,
the palettes in `hcl.colors()` are a path within HCL space.  For each
...
#| fig.align: 'center'
#| fig.pos: 'ht!'
#| fig.cap: 'Hue, Chroma, and Luminance paths for the `"Blues 3"` palette.'
library("colorspace")
...
The details always depend a lot on how many colors you need. For
example, a palette with light gray, medium color, and full color may still
work great on a white background if you are not interested much in the
light gray group and just want them to be around in the background for
reference.
...
the \pkg{wesanderson} package (CITE), `"Cividis"`,
