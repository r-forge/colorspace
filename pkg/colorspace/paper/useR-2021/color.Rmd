...
base R provided shockingly few functions for choosing colors from readily available palettes. And the palettes
...
- Better palettes were easily available in packages on CRAN (Comprehensive R Archive Network)
  starting with \CRANpkg{RColorBrewer} [@color:RColorBrewer], later
...
- `palette.colors()` provides a new improved default color palette as well as a few other well-established qualitative palettes [@color:grDevices2]
- `hcl.colors()` provides a wide range of qualitative, sequential, and diverging palettes obtained by a standardize approach in
  the so-called HCL (hue-chroma-luminance) space [@color:Wiki+HCL], see @color:grDevices and @color:Zeileis+Fisher+Hornik:2020.
  Most of these HCL palette approximate very closely well-established palettes from \pkg{RColorBrewer}, \pkg{viridis}, \pkg{rcartocolor},
...
`Okabe-Ito` [@color:Okabe-Ito:2008] was designed to be very robust under color vision deficiencies. As it uses somewhat different hues than the old `R4` palette
...
The other palettes are from well-established software packages like \pkg{ggplot2} [@color:ggplot2], ColorBrewer.org [@color:Harrower+Brewer:2003],
or Tableau [@color:Tableau]. Moreover, two palettes with many colors (36 and 26, respectively) are used from the
\CRANpkg{Polychrome} package [@color:Polychrome].
...
Additionally, the figure uses @color:Okabe+Ito:2008 palette and it switches from a color legend to direct labeling.
...
HCL makes it easy to describe colors for humans, using the three basic properties hue, chroma, and luminane (see Figure \ref{fig:hcl}).
...
was suggested by @color:Ihaka:2003 and is implemented in `hcl.colors()`. See left panel of Figure \ref{fig:hclpal}
...
Might have contributed to Trump's confusion in "Sharpiegate" incident. See @color:Zeileis+Stauffer:2019 for a more detailed discussion and other alternative color palettes.
