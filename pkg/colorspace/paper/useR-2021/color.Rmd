...
* Better palettes became available via packages on CRAN 
  ...
  starting with \CRANpkg{RColorBrewer} [@RColorBrewer],
  later
  ...
  among many others.  In most cases, these made palettes available in R that
  were developed elsewhere, for example, 
  [ColorBrewer.org](https://colorbrewer2.org/) [@Harrower+Brewer:2003], 
  [CARTOColors](https://carto.com/carto-colors/) [@CARTOColor],
  Fabio Crameri's scientific color maps [@Crameri+Shephard+Heron:2020],
  and the Viridis palette for `matplotlib` in Python [@Smith+VanDerWalt:2015].
...
\pkg{RColorBrewer} package and that includes a facility for 
...
Both `"Hawaii"` and `"Batlow"` also span a large range of hues
(a "scientific rainbow").
...
while `"Hawaii"` and `"Batlow"` are for use with a white/light background.
