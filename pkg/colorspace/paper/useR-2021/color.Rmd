...
```{r dorian, fig.cap = ifelse(knitr::is_html_output(), 'Probability of wind speeds $>$ 39 mph (63 km h$^{-1}$) during hurricane Dorian in 2019. On the left is the the original image (top row) and two reproductions using the `"Reds 3"` single-hue sequential palette (middle) and `"YlGnBu"` multi-hue sequential palette (bottom). On the right are emulations of how the images on the left might appear to a colorblind viewer.', 'Probability of wind speeds $>$ 39\\,mph (63\\,km\\,h$^{-1}$) during hurricane Dorian in 2019. On the left is the the original image (top row) and two reproductions using the \\code{"Reds 3"} single-hue sequential palette (middle) and \\code{"YlGnBu"} multi-hue sequential palette (bottom). On the right are emulations of how the images on the left might appear to a colorblind viewer.')}
#| echo: FALSE
#| out.width: '100%'
#| fig.width: 6
#| fig.height: 2.5
...
ggdraw() + 
  draw_image("images/dorian-useR-2021-reds3.png", width = 0.5) +
  draw_image("images/dorian-useR-2021-reds3-deutan.png", width = 0.5, x = 0.5)
