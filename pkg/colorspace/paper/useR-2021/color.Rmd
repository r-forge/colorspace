...
abstract: >
  ...
  the perceptual hue, chroma, luminance (HCL) properties in the palette. 
  This paper provides an overview of the new color palettes and the new color palette
  functions along with advice about which palettes are appropriate for specific tasks,
...
#| fig.align: 'center'
#| fig.pos: 'h!'
#| fig.cap: 'Typical usage of color for coding qualitative/categorical information (left) and quantiative/continuous information (right). Left: Scatter plot of weekly gas consumption by outside temperature before and after installing a house insulation. Right: Choropleth map of median income in the 16 regions of New Zealand in 2018.'
library("ggplot2")
...
#| fig.keep: 'last'
#| fig.pos: 'ht!'
#| fig.cap: 'Old base R palettes.  At top left is the old default palette, consisting largely of highly saturated primary colors or combinations thereof.  Below that is the rainbow palette of different highly saturated hues.  The middle column shows the old sequential palettes, with heat colors again being highly saturated.  The last column shows an old diverging palette plus two palettes motivated by shadings of geographic maps.'
...
The output below shows what R would produce prior to version 4.0.0.
A *swatch* of color circles is included below the output.
...
of colorblindness (nearly 10% of the male population).
...
selecting colors from is *not* a fixed set of colors.  Instead,
...
palette `"YlGnBu"` and nine colors from the diverging palette
`"Purple-Green"`. 
...
`"Reds 3"` that can be selected using `hcl.colors()` and the
...
#| fig.pos: 'ht!'
#| fig.show: 'hold'
#| fig.cap: 'Probability of wind speeds $>$ 39\,mph (63\,km\,h$^{-1}$) during hurricane Dorian in 2019.  Top-left is the original image and top-right is an emulation of how the original image might appear to a colorblind viewer.  Middle-left is a reproduction using the `"Reds 3"` single-hue sequential palette, with a colorblind emulation at middle-right.  Bottom-left is a reproduction using the `"YlGnBy"` multi-hue sequential palette, with emulation at bottom-right.'
library("cowplot")
ggdraw() + 
...
ggdraw() + 
  draw_image("images/dorian-reds3.png", width = 0.5) +
  draw_image("images/dorian-reds3.png", width = 0.5, x = 0.5)
...
The `"Reds 3"` version of the image conveys the information more clearly
by relating the monotonic changes in probability to monotonic
changes in luminance (hue is constant and
the saturation peaks towards the middle, which is very similar to the
`"Blues 3"` palette shown in 
Figure&nbsp;\@ref(fig:blues3hcl), just with a different hue).
The deuteranope version retains this advantage.

The `"YlGnBu"` version of the image is also more effective than
the original.  This palette varies hue as well as luminance (chroma stays
reasonably constant; see Figure&nbsp;\@ref(fig:ylgnbuhcl)), 
which makes individual colors in the 
palette more distinguishable, which has the advantage of making
it easier to associate contour bands with the legend.  This version
...
```{r ylgnbuhcl, fig.cap = ifelse(knitr::is_html_output(), 'Hue, Chroma, and Luminance paths for the `"YlGnBu"` palette. This plot is created by the `colorspace::specplot()` function.  We can see that luminance increases monotonically and hue is also varied, while chroma wanders around a bit.', 'Hue, Chroma, and Luminance paths for the \\code{"YlGnBu"} palette. This plot is created by the \\code{colorspace::specplot()} function.  We can see that luminance increases monotonically and hue is also varied, while chroma wanders around a bit.')}
#| echo: FALSE
#| fig.width: 5
#| fig.height: 4
#| out.width: '60%'
...
#| fig.pos: 'ht!'
library("colorspace")
specplot(hcl.colors(100, "YlGnBu"))
```
...
some suggestions for robust default palettes are given in the
[Summary Section](#Summary).

# A gallery of palettes

This section goes through all of the color palettes that are now
available in R (without using any additional packages).
...
As previously mentioned, this is a qualitative palette with
...
to choose from.  For example, the old R default palette is available via the
name `"R3"`, though this palette is really only useful to reproduce old
behavior and remind us how bad things used to be.  The `palette.pals()`
...
it depends on the luminance range how effective this is.
...
are provided by the package \pkg{colorblindcheck} [@colorblindcheck].
...
colors from a fixed set.  Furthermore, the `hcl.colors()` palettes
are only emulations based on the original ColorBrewer.org palettes
and have subtly different properties.
...
when we need to match specific colours to a legend 
...
designed for perceptual balance, so that no color has a greater
...
vary in the same way as we move from the central neutral colour towards
either end of the palette.
Figure&nbsp;\@ref(fig:purplegreenhcl) shows this idea of balance for
the `"Purple-Green"` palette.

```{r purplegreenhcl, fig.cap = ifelse(knitr::is_html_output(), 'Hue, Chroma, and Luminance paths for the `"Purple-Green"` palette. This plot is created by the `colorspace::specplot()` function.  We can see that the palette is "balanced"; luminance and chroma vary symmetrically about the central neutral color.', 'Hue, Chroma, and Luminance paths for the \\code{"Purple-Green"} palette. This plot is created by the \\code{colorspace::specplot()} function.  We can see that the palette is "balanced"; luminance and chroma vary symmetrically about the central neutral color.')}
#| echo: FALSE
#| fig.width: 5
#| fig.height: 4
#| out.width: '60%'
...
#| fig.pos: 'ht!'
library("colorspace")
specplot(hcl.colors(100, "Purple-Green"))
```
...
For example, Figure&nbsp;\@ref(fig:fallhcl) demonstrates this issue
with the `"Fall"` palette.

```{r fallhcl, fig.cap = ifelse(knitr::is_html_output(), 'Hue, Chroma, and Luminance paths for the `"Fall"` palette. This plot is created by the `colorspace::specplot()` function.  We can see that the palette is "unbalanced"; the green arm of the palette has much lower chroma than the right arm of the palette.', 'Hue, Chroma, and Luminance paths for the \\code{"Fall"} palette. This plot is created by the \\code{colorspace::specplot()} function.  We can see that the palette is "unbalanced"; the green arm of the palette has much lower chroma than the right arm of the palette.')}
#| echo: FALSE
#| fig.width: 5
#| fig.height: 4
#| out.width: '60%'
...
which is a colorblind-safe version of `"Viridis"` (from 
...
Package developers may also benefit from the new palettes available
...
: An overview of the new palette functionality:  For each main type of palette, the _Purpose_ row describes what sort of data the type of palette is appropriate for, the _Generate_ row gives the functions that can be used to generate palettes of that type, the _List_ row names the functions that can be used to list available palettes, and the _Robust_ row identifies one or two good default palettes of that type. 
...
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
|_Robust_  | `"Okabe-Ito"`, `"R4"`                 | `"Blues 3"`, `"YlGnBu"`             | `"Purple-Green"`, `"Blue-Red 3"`              |
|          |                                       |                                     |                                               |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
