...
the new `"R4"` default and a new `"Okabe-Ito"` qualitative 
palette [@Okabe+Ito:2008]
that can be selected using `palette.colors()`.
...
The second row uses a new sequential palette,
...
: When we are using color to distinguish between the levels of a categorical
  ...
  though this will not print well in greyscale
...
be appropriate for greyscale printing.
...
and to work for greyscale printing.  Finally, the palettes have been
