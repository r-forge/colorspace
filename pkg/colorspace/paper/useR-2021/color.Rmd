...
* Better palettes became available via packages on CRAN 
  ...
  \CRANpkg{viridis} [@viridis], \CRANpkg{rcartocolor} [@rcartocolor], or 
  \CRANpkg{scico} [@scico],
  among many others.
...
Figure \ref{fig:viridishcl}.  The palette is constant in the hue
dimension, so the palette consists entirely of different shades of blue.
...
```{r viridishcl, echo=FALSE, fig.width=5, fig.height=4, out.width='.8\\textwidth', fig.align='center', fig.pos='t!', fig.cap='Hue, Chroma, and Luminance paths for the `"Blues 3"` palette.'}
library(colorspace)
...
a data visualisation is attempting to make.  For example, perceptual
...
The common feature of these palettes are that they mainly vary hue. 
...
as green for "nature" and red for "temperature".  
...
The top group of palettes, starting with `"Reds"`, `"Greens"`,
