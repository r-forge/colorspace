...
abstract: >
  ...
  This paper provides an overview of these new color functions and the palettes they provide
...
* Better palettes became available via packages on CRAN 
  ...
  c [@Ihaka:2003; @Zeileis+Hornik+Murrell:2009], and 
  more recently
...
Figure&nbsp;\@ref(fig:r4r3hcl) 
compares the new `"R4"` default palette with the old
`"R3"` default palette by showing the hue, chroma, and luminance values 
for both palettes.  This clearly shows that the hues are almost identical,
the luminance range is narrower for the new default (especially if we exclude
black), and the chroma values are much lower and less varied
for the new default (again, especially if we exclude black and grey).
We will use "spectrum" plots like this one to explore
and compare different palettes throughout the document.

```{r r4r3hcl, fig.cap = ifelse(knitr::is_html_output(), 'Hue, chroma, and luminance paths for both the new (`"R4"`, dashed lines) and old (`"R3"`, solid lines) palettes. This plot is created by the `colorspace::specplot()` function.  We can see that the hues are almost identical, while the `"R4"` palette has less variability in luminance and significantly lower chroma.', 'Hue, chroma, and luminance paths for both the new (\\code{"R4"}, dashed lines) and old (\\code{"R3"}, solid lines) palettes. This plot is created by the \\code{colorspace::specplot()} function.  We can see that the hues are almost identical, while the \\code{"R4"} palette has less variability in luminance and significantly lower chroma.')}
#| echo: FALSE
#| fig.width: 5
#| fig.height: 4.5
#| out.width: '49%'
...
#| fig.pos: 'ht!'
library("colorspace")
specplot(palette.colors(8, "R3"), palette.colors(8, "R4"))
```
...
```{r dorian, fig.cap = ifelse(knitr::is_html_output(), 'Probability of wind speeds $>$ 39 mph (63 km h$^{-1}$) during hurricane Dorian in 2019. On the left is the the original image (top row) and two reproductions using the `"Reds"` single-hue sequential palette (middle) and `"YlGnBu"` multi-hue sequential palette (bottom). On the right are emulations of how the images on the left might appear to a colorblind viewer.', 'Probability of wind speeds $>$ 39\\,mph (63\\,km\\,h$^{-1}$) during hurricane Dorian in 2019. On the left is the the original image (top row) and two reproductions using the \\code{"Reds"} single-hue sequential palette (middle) and \\code{"YlGnBu"} multi-hue sequential palette (bottom). On the right are emulations of how the images on the left might appear to a colorblind viewer.')}
#| echo: FALSE
#| out.width: '100%'
#| fig.width: 6
#| fig.height: 2.5
...
ggdraw() + 
  draw_image("images/dorian-useR-2021-reds.png", width = 0.5) +
  draw_image("images/dorian-useR-2021-reds-deutan.png", width = 0.5, x = 0.5)

ggdraw() + 
  draw_image("images/dorian-useR-2021-ylgnbu.png", width = 0.5) + 
  draw_image("images/dorian-useR-2021-ylgnbu-deutan.png", width = 0.5, x = 0.5)
```
...
the original. This palette varies hue as well as luminance with higher
chroma for the dark colors and lower chroma for the light colors
...
Note that the `"YlGnBu"` palette is very similar to the very popular
...
```{r ylgnbu-viridis, fig.cap = ifelse(knitr::is_html_output(), 'Hue, chroma, and luminance paths for the `"YlGnBu"` (left) and `"Viridis"` (right) palettes. These plots are created by the `colorspace::specplot()` function. For `"YlGnBu"` we can see that hue changes from blue to yellow, luminance increases monotonically, and chroma has a small peak in the middle and then decreases with luminance. `"Viridis"`, on the other hand, has almost the same hue and luminance trajectory but chroma increases for the light colors, yielding a palette with high chroma throughout.', 'Hue, chroma, and luminance paths for the \\code{"YlGnBu"} (left) and \\code{"Viridis"} (right) palettes. These plots are created by the `colorspace::specplot()` function. For \\code{"YlGnBu"} we can see that hue changes from blue to yellow, luminance increases monotonically, and chroma has a small peak in the middle and then decreases with luminance. \\code{"Viridis"}, on the other hand, has almost the same hue and luminance trajectory but chroma increases for the light colors, yielding a palette with high chroma throughout.')}
#| echo: FALSE
#| fig.width: 5
#| fig.height: 4.5
#| out.width: '49%'
...
to choose from, e.g., `"R4"` specifies the new R default palette 
...
certain trajectories of the perceptual properties hue, chroma, and luminance (HCL),
are provided by `hcl.colors()`. Most of the resulting palettes have one
...
`colorspace::sequential_hcl()` and `colorspace::diverging_hcl()` 
to generate even further palettes by defining a custom set of
...
For example, Figure&nbsp;\@ref(fig:purplegreen-fall) (right) demonstrates this issue
...
which is an even more colorblind-safe version of `"Viridis"` (from 
