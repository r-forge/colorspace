...
abstract: >
  Since R 4.0.0 R has had a new and much improved default palette.
  In addition, a selection of
  more than 100 well-established palettes are now provided via
  the function `palette.colors()` and `hcl.colors()`.
preamble: |
...
#| fig.align: 'center'
#| fig.pos: 'h!'
#| fig.cap: 'Typical usage of color for coding qualitative/categorical (left) and quantiative/continuous (right) information. Left: Scatter plot of weekly gas consumption by outside temperature before and after installing a house insulation. Right: Choropleth map of median income in the 16 regions of New Zealand in 2018.'
library("ggplot2")
...
```{r oldPalettes, fig.width=6, fig.height=1.5}
#| echo: FALSE
#| out.width: '100%'
#| fig.align: 'center'
#| fig.keep: 'last'
#| fig.pos: 'ht!'
#| fig.cap: 'Old base R palettes.'
...
See also Figure&nbsp;\@ref(fig:oldPalettes)
that depicts the old default `palette()` along
...
* `rainbow()` implements the infamous rainbow (or "jet") color palette 
  that was very widely used for all types of variables
  (possibly after restricting 
...
To illustrate the problems with the old default palette,
...
the old `"R3"` default color palette with
the new `"R4"` default and anoter new qualitative palette
known as  `"Okabe-Ito"` [@Okabe+Ito:2008]
that can all be selected using `palette.colors()`.
...
[based on the algorithms of @Machado+Oliveira+Fernandes:2009].
The second row uses the `"R4"` palette and the third row 
uses `"Okabe-Ito"`; both using direct labels for the different time series instead of
a color legend.

We can see that the `"R3"` colors are highly saturated and they vary in 
luminance. For example, the cyan line is noticeably lighter than the others.
Futhermore, for deuteranope viewers, the CAC and the SMI lines are difficult to 
...
The `"Okabe-Ito"` palette also works better than `"R3"`,
particularly for deuteranope viewers.
...
To illustrate an application of sequential color palettes,
...
The second row uses the sequential palette
`"OrRd"` that can be selected using `hcl.colors()`.
...
The `"OrRd"` version of the image conveys the information more clearly
by relating the monotonic changes in probability to both monotonic
changes in luminance 
and monotonic changes in saturation.  The deuteranope version retains both
of these qualities.
...
#| fig.pos: 'ht!'
#| fig.show: 'hold'
#| fig.cap: 'Probability of wind speeds $>$ 39\,mph (63\,km\,h$^{-1}$) during hurricane Dorian in 2019.'
library("cowplot")
ggdraw() + 
  draw_image("images/dorian-useR-2021-orig.png", width = 0.5) + 
  draw_image("images/dorian-useR-2021-orig-deutan.png", width = 0.5, x = 0.5)

ggdraw() + 
  draw_image("images/dorian-useR-2021-orrd.png", width = 0.5) + 
  draw_image("images/dorian-useR-2021-orrd-deutan.png", width = 0.5, x = 0.5)
```

## Overview and Guidance

With the palettes that are now available as part of the core R graphics systems,
we have a lot more choice, but we also have some good robust
palettes for the most common use cases.

**Qualitative palettes**
: When we are using color to distinguish between the levels of a categorical
  variable, we want a palette that has clear changes in hue.
  We can generate a palette with `palette.colors()` and 
  `palette.pals()` gives the list of available palettes.
  We can also generate a palette with `hcl.colors()`, provided
  we use one of the palettes listed by `hcl.pals("qualitative")`.
  A good choice that also accommodates colorblind viewers is
  `"Okabe-Ito"`.  When filling large areas, such as bars of a bar plot,
  something with higher luminance, like `"Pastel 1"` may be more appropriate,
  though this will not print well in grayscale
  nor work well for colorblind viewers.
  ...
  #| fig.height: 1.5
  #| out.width: '100%'
  paletteGrids(list('palette.colors(palette="Okabe-Ito")'=palette.colors(palette="Okabe-Ito"), NULL, NULL, NULL, NULL, NULL, 'hcl.colors(9, palette="Pastel 1")'=hcl.colors(9, palette="Pastel 1")), qual=TRUE)
  ```

**Sequential palettes**
: When we are using color to encode numeric (or ordered) information,
  we want a palette that has clear changes in luminance and/or chroma.
  We can generate palettes with `hcl.colors()`, provided 
  we use one of the palettes listed by `hcl.pals("sequential")`.
  A simple solution is a single-hue palette like `"Greens 3"`.
  A greater range of distinguishable colors can be obtained by selecting
  a multi-hue palette like `"Viridis"`. This can also be useful to
  emphasise extreme values.
  ...
  #| fig.height: 1.5
  #| out.width: '100%'
  paletteGrids(list('hcl.colors(9, palette="Greens 3")'=hcl.colors(9, palette="Greens 3"), NULL, NULL, NULL, NULL, NULL, 'hcl.colors(9, palette="Viridis")'=hcl.colors(9, palette="Viridis")), qual=FALSE)
  ```

**Diverging palettes**
: When we are using color to encode a numeric variable with a central
  or neutral reference value, we need changes in lumincance and/or chroma
  in two directions away from a central neutral color, with a different
  hue for each direction.
  We can generate palettes with `hcl.colors()`, provided 
  we use one of the palettes listed by `hcl.pals("diverging")`
  or `hcl.pals("divergingx")`.
  Examples of good choices are `"Purple-Green"` and `"Blue-Red3"`.
  ...
  #| fig.height: 1.5
  #| out.width: '100%'
  paletteGrids(list('hcl.colors(9, palette="Purple-Green")'=hcl.colors(9, palette="Purple-Green"), NULL, NULL, NULL, NULL, NULL, 'hcl.colors(9, palette="Blue-Red3")'=hcl.colors(9, palette="Blue-Red3")), qual=FALSE)
  ```
  
The following sections describe the full range of
new color palettes in more detail.
...
+:=========+:======================================+:====================================+:==============================================+
| Purpose  | Categorical                           | Ordered or numeric                  | Ordered or numeric with center                |
|          |                                       | (high $\rightarrow$ low)            | (high $\leftarrow$ neutral $\rightarrow$ low) |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
| Function | `palette.colors()`,                   | `hcl.colors()`                      | `hcl.colors()`                                |
|          | `hcl.colors()`                        |                                     |                                               |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
| Listing  | `palette.pals()`,                     | `hcl.pals("sequential")`            | `hcl.pals("diverging")`,                      |
|          | `hcl.pals("qualitative")`             |                                     | `hcl.pals("divergingx")`                      |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
| Robust   | `"Okabe-Ito"`                         | Most, e.g.,                         | `"Purple-Green"`, `"Blue-Red 3"`              |
|          |                                       | `"Viridis"`, `"YlnGnBu"`            |                                               |
+----------+---------------------------------------+-------------------------------------+-----------------------------------------------+
...
The new, "R4", default palette is shown at top-left of 
Figure&nbsp;\@ref(fig:newPalettes).
More information about the construction of the new default palette
is given in @grDevices2.

```{r newPalettes, fig.cap = ifelse(knitr::is_html_output(), 'New base R palettes available from the `palette.colors()` function.  The label above each swatch shows the argument to provide to `palette.colors()` to produce the set of colors.  The palette at top-left is the new default that is also produced by `palette()`.  The `"Okabe-Ito"` palette is the default that is produced by `palette.colors()` (with no arguments).', 'New base R palettes available from the \\code{palette.colors()} function.  The label above each swatch shows the argument to provide to \\code{palette.colors()} to produce the set of colors.  The palette at top-left is the new default that is also produced by \\code{palette()}.  The \\code{"Okabe-Ito"} palette is the default that is produced by \\code{palette.colors()} (with no arguments).')}
#| echo: FALSE
#| fig.width: 6
#| fig.height: 4
#| fig.keep: 'last'
...
(see Figure&nbsp;\@ref(fig:newPalettes)).
...
The `palette` argument to the `palette.colors()` function allows us to select a
different palette.  For example, the old R default palette is available via the
name `"R3"`, though this palette is really only useful to reproduce old
behaviour and remind us how bad things used to be.  The `palette.pals()`
function returns a character vector of the available palette names and 
Figure&nbsp;\@ref(fig:newPalettes) shows swatches for each of the
available palettes.
...
including sequential and diverging palettes, which are more appropriate
for representing continuous variables.  
As with the `palette.colors()` function, the first
...
selecting colors from is *not* a fixed set of colours.  Instead,
...
dimension, so the palette consists entirely of different shades of 
...
colourful towards the middle of the palette.

```{r blues3hcl, fig.cap = ifelse(knitr::is_html_output(), 'Hue, Chroma, and Luminance paths for the `"Blues 3"` palette.', 'Hue, Chroma, and Luminance paths for the \\code{"Blues 3"} palette.')}
#| echo: FALSE
#| fig.width: 5
#| fig.height: 4
#| out.width: '60%'
...
#| fig.pos: 'ht!'
library("colorspace")
specplot(hcl.colors(100, "Blues 3"))
```

Because the palettes from `hcl.colors()` are based on a continuous
path in HCL space, we can select as many colors as we like.

Most of the palettes that are provided by `hcl.colors()`
have one or more desirable perceptual properties:
...
* **Perceptual balance:**  This means that, for example,
  ...
  pretty much the same, so no colour stands out from the others.  
...
want some colours to have a greater visual impact than others.
The details always also depend a lot on how many colors you need. For
example, a palette with light gray, medium color, and full color may still
work great on a white background if the
light gray group is less important 
is just provided in the background for
reference.

The following sections list the palettes available via `hcl.colors()`,
broken down into qualitative, sequential, and diverging palettes.
...
The first five palettes are close to the ColorBrewer.org 
palettes of the same name. 
They employ different levels of chroma and luminance and
span the full hue range. 
Most of these palettes are also available as a fixed set
of colors via `palette.colors()`;  the difference is that
...
of the ColorBrewer-inspired
palettes because, for example, they are not all colorblind-safe.
...
if all categories in a variable have equal importance, but they
will not perform well for colorblind viewers and they will not 
...
Single-hue sequential palettes vary only from dark to light, with
a constant underlying hue.  Certain hues will be more 
...
colourful to less.  The result is a palette that makes it very easy to
...
#| fig.align: 'center'
#| fig.pos: 'ht!'
#| fig.cap: 'Single-hue sequential palettes.'
...
last palette, `"Oslo"`, is taken from the scientific color maps of 
the \pkg{scico} package
 and is intended for a black/dark background and hence the order
is reversed starting from a light blue (not a light gray).
...
(e.g., `"Light Grays"` or `"Blues 2"`).

### Multi-hue sequential palettes

Multi-hue
sequential palettes not only vary luminance, from light to dark, but 
also vary hue (and in some cases chroma);  this can be useful
...
throughout the palette (e.g., emulations of palettes from
the \pkg{viridis} and \pkg{rcartocolors} packages). 
...
#| fig.align: 'center'
#| fig.pos: 'ht!'
#| fig.cap: 'Multi-hue sequential palettes.'
single <- rownames(hcl_palettes("sequential (single-hue)"))
pals <- hcl.pals("sequential")
...
`"Red-Blue"` to `"Terrain 2"` are various palettes created during the
...
`"Mint"` to `"Sunset"` closely match palettes provided in the
\pkg{rcartocolors} package.
...
small number of ordered values, e.g., three-to-four. 
...
args <- c(args[c('"Reds"', '"Greens"', '"Blues"')],
          ...
          list(NULL, NULL, NULL),
          args[c('"Lajolla"', '"Hawaii"', '"Batlow"')],
          args[c('"Turku"')], list(NULL, NULL))
paletteGrids(args, qual=FALSE)
```
...
sequential palette because chroma monotonically decreases across
the palette.

The next group of palettes, `"Lajolla"` to `"Batlow"` closely match the
palettes of the same name from the \pkg{scico} package.
These palettes are constructed with a luminance scale 
so that there is a clear visual ordering of the palette.
They are also designed to be readable by colourblind users
and to work for grayscale printing.  Finally, the palettes have been
designed for perceptual balance, so that no colour has a greater
visual emphasis than any other.
Both `"Lajolla"` and `"Turku"`
are intended for use with a black/dark background.
...
args <- c(args[c('"Green-Orange"', '"Blue-Yellow 3"', '"Cyan-Magenta"',
          ...
          list(NULL, NULL, NULL),
          args[c('"Broc"', '"Vik"', '"Lisbon"',
                 '"Cork"', '"Berlin"', '"Tofino"')])
paletteGrids(args, qual=FALSE)
```

Almost all of the palettes in the first two groups, those
involving simple colour pairs like
...
which closely matches the palette of the same name from the
\pkg{rcartocolors} package.  

The palettes `"Broc"` to `"Vik"` and `"Berlin"` to `"Tofino"` closely
match the scientific color maps from the \pkg{scico} package,
...
light yellow). Consequently, the chroma/luminance within these palettes
...
#| fig.align: 'center'
#| fig.pos: 'ht!'
#| fig.cap: 'Flexible diverging palettes.'
...
from the \pkg{rcartocolors} package.
...
which is a colourblind-safe version of `"Viridis"` (from 
the \pkg{viridis} package) and
`"Roma"`, which closely matches the palette of the same name 
from the \pkg{scico} package.


# New defaults in graphical functions

The default colour palette will be most visible in the output from
...
`2` or `3`.
The resulting colours will still be similar to the old output, e.g.,  
still a red or a green, but just a different shade.

Moreover, a couple of functions explicitly have new defaults, 
namely: `image()` and `filled.contour()`, now using sequential `YlOrRd` palette
(from ColorBrewer) 
which uses similar hues as the old `heat.colors()`. 
See Figure&nbsp;\@ref(fig:graphics).

Finally, the `hist()` and `boxplot()` functions (and therefore
calls of the form `plot(num ~ factor, ...)`, also have a new default color:
...
#| fig.align: 'center'
#| fig.pos: 'ht!'
#| fig.cap: 'New defaults in base graphics.'
par(mfrow = c(1, 3))
image(volcano)
...
The default colour palette in R has been improved since R version 4.0.0.
...
and sequential and diverging palettes for use with continuous data. 
